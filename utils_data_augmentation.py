...
def augment_data(train_size,Images,groundtruths, patch_size,save_path):
    ...
    count = 0 
    print ('you are having a patch size of' , patch_size[0]  )
    aug_groundtruth = {}
    for img in tqdm(Images):
        if patch_size[0] == 64 or patch_size[0]==32 :
            image = (cv2.resize(cv2.imread(img,-1),(0,0), fx=0.5, fy=0.5) *1.0 / 65535.0 ).astype('float32')
        else:
